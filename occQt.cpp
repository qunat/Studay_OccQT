    ...
    : QMainWindow(parent)
{
    ...
    //myOccView->setGeometry(0,0,300,300);
    myOccView->reset();

    myOccView->show();
    int a;
    //a=this->geometry().width();
    //printf("%d",a);
...
void occQt::createActions( void )
{
    ...
    // Primitive
    connect(ui.actionBox, SIGNAL(triggered()), this, SLOT(offset()));
...
void occQt::offset(void)
{
	gp_Pnt aPnt1(10, 10, 0);
	gp_Pnt aPnt2(10, -10, 0);
	gp_Pnt aPnt3(-10, -10, 0);
	gp_Pnt aPnt4(-10, 10, 0);
	gp_Pnt aPnt5(0, 20, 0);
	gp_Pnt aPnt6(3, 10, 0);
	...
	//Handle(Geom_TrimmedCurve) aSegment5 = GC_MakeSegment(aPnt5, aPnt6);
	//Handle(Geom_TrimmedCurve) aSegment6 = GC_MakeSegment(aPnt6, aPnt1);
	...
	//TopoDS_Edge aEdge5 = BRepBuilderAPI_MakeEdge(aSegment5);
	//TopoDS_Edge aEdge6 = BRepBuilderAPI_MakeEdge(aSegment6);

	TopoDS_Wire aWire1 = BRepBuilderAPI_MakeWire(aEdge1, aEdge2, aEdge3, aEdge4);
	//TopoDS_Wire aWire2 = BRepBuilderAPI_MakeWire(aWire1, aEdge5);
	//TopoDS_Wire aWire3 = BRepBuilderAPI_MakeWire(aWire2, aEdge6);
	...
	Handle(Geom_TrimmedCurve) aSegment7 = GC_MakeSegment(aPnt7, aPnt8);
	Handle(Geom_TrimmedCurve) aSegment8 = GC_MakeSegment(aPnt8, aPnt9);
	Handle(Geom_TrimmedCurve) aSegment9 = GC_MakeSegment(aPnt9, aPnt10);
	Handle(Geom_TrimmedCurve) aSegment10 = GC_MakeSegment(aPnt10, aPnt7);

	TopoDS_Edge aEdge7 = BRepBuilderAPI_MakeEdge(aSegment7);
	TopoDS_Edge aEdge8 = BRepBuilderAPI_MakeEdge(aSegment8);
	TopoDS_Edge aEdge9 = BRepBuilderAPI_MakeEdge(aSegment9);
	TopoDS_Edge aEdge10 = BRepBuilderAPI_MakeEdge(aSegment10);

	TopoDS_Wire aWire4 = BRepBuilderAPI_MakeWire(aEdge7, aEdge8, aEdge9, aEdge10);

	TopoDS_Face aFace2 = BRepBuilderAPI_MakeFace(aWire4);
	TopoDS_Shape aCut = BRepAlgoAPI_Cut(aFace1, aFace2);



	TopExp_Explorer ex;

	Handle(AIS_Shape)  anCut = new AIS_Shape(aFace1);
	myOccView->getContext()->Display(anCut, Standard_True);

	

	/*python参考代码
	BRepFill_OffsetWire offset;
	...
	//const TopoDS_Face curFace;
	BRepOffsetAPI_MakeOffset offset;
	offset.Init(GeomAbs_Arc);

	for (ex.Init(aFace1, TopAbs_WIRE); ex.More(); ex.Next())
	{
		const TopoDS_Wire curWire = TopoDS::Wire(ex.Current());
		offset.AddWire(curWire);
	}

	
	for (int i = 1; i <= 50; i++)
	{
		offset.Perform(-0.5*i);
		TopoDS_Shape S = offset.Shape();
		Handle(AIS_Shape)  anOffset = new AIS_Shape(S);
		myOccView->getContext()->Display(anOffset, Standard_True);
	}
